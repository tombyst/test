...
const resx2js = require("gulp-resx2js");
...
gulp.task("thirdparty", function () {
  const locales = ["fr", "vi"]; // specify locales to include with moment
  const scripts = [
    "js/polyfills.js",
    "js/fitty.min.js",
  ...
  locales.forEach(function (x) {
    scripts.push("js/momentlocales/" + x + ".js");
  });
...
gulp.task("css", function () {
  const faCSS = gulp
    ...
    .pipe(replace(/url\((')?\.\.\/webfonts\//g, "url($1/dist/webfonts/"));

  const bsCSS = gulp.src("node_modules/bootstrap/dist/css/bootstrap.min.css");
  ...
  (combinedCss = es.concat(faCSS, bsCSS, mainCSS).pipe(concat("css.css"))),
    (faFontFiles = gulp.src(
      "./node_modules/@fortawesome/fontawesome-free/webfonts/*",
      { base: "./node_modules/@fortawesome/fontawesome-free/" }
    ));

  return es.concat(combinedCss, faFontFiles).pipe(gulp.dest("./dist/"));
});

gulp.task("scripts-body", function () {
  return gulp
    .src(["js/bower/jquery.magnific-popup.min.js"])
